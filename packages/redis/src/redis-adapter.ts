...
class Backend {
	constructor(readonly runner: RedisRunner, readonly keys: RedisKeys) {}
	...
	async pipeline(commands: Array<{ command: string; args?: RedisArgument[] }>): Promise<unknown[]> {
		...
		if (!Array.isArray(output) || output.length !== commands.length) throw new TypeError('Redis pipeline returned an invalid result shape.');
		for (const result of output) {
			if (result instanceof Error) throw result;
			if (Array.isArray(result) && result.length === 2 && result[0] instanceof Error) throw result[0];
		}
		return output;
	}
...
export function redis(runner: RedisRunner, options: RedisOptions = {}): PersistenceAdapter {
	...
	return {
		async migrate() {
			...
			if (stored == null) await backend.command('HSETNX', [backend.keys.meta(), 'schemaVersion', FLUE_SCHEMA_VERSION]);
...
async function inspectServer(backend: Backend, enabled: boolean): Promise<void> {
	...
	if (clusterEnabled === undefined) throw new TypeError('@flue/redis could not verify that Redis Cluster is disabled.');
	...
	if (policy !== 'noeviction') throw new TypeError('@flue/redis requires maxmemory-policy noeviction.');
}

async function stageHash(backend: Backend, key: string, fields: Array<[string, string]>): Promise<void> {
...
async function reclaimGenerations(
	...
	const removed = strings(await backend.eval(reclaimGenerationsScript, [pointer, readers, generations], [force ? Number.MAX_SAFE_INTEGER : Date.now() - GENERATION_GRACE_MS, 100]));
...
class RedisSessionStore implements SessionStore {
	...
	async save(id: string, data: SessionData): Promise<void> {
		...
		await this.backend.eval(publishGenerationScript, [pointer, generationKey, generations], [generation, Date.now()]);
		await reclaimGenerations(this.backend, pointer, this.backend.keys.sessionReaders(id), generations, value => this.backend.keys.sessionGeneration(id, value));
	}

	async load(id: string): Promise<SessionData | null> {
		return readGeneration(this.backend, this.backend.keys.session(id), this.backend.keys.sessionReaders(id), value => this.backend.keys.sessionGeneration(id, value), record => {
			if (!record.session) throw new TypeError('Persisted Redis session generation is malformed.');
	...
	async delete(id: string): Promise<void> {
		...
		await reclaimGenerations(this.backend, this.backend.keys.session(id), this.backend.keys.sessionReaders(id), generations, value => this.backend.keys.sessionGeneration(id, value), true);
		await this.backend.command('DEL', [this.backend.keys.session(id), generations, this.backend.keys.sessionReaders(id)]);
	}
}
...
function createChunkStore(backend: Backend): PersistedChunkStore<Promise<void>> {
	return {
		async read(owner) {
			return await readGeneration(backend, backend.keys.chunkOwner(owner), backend.keys.chunkReaders(owner), value => backend.keys.chunkGeneration(owner, value), record => {
		...
		async replace(owner, chunks) {
			...
			await backend.eval(publishChunksScript, [pointer, generationKey, generations, backend.keys.chunkOwners()], [generation, Date.now()]);
			await reclaimGenerations(backend, pointer, backend.keys.chunkReaders(owner), generations, value => backend.keys.chunkGeneration(owner, value));
		},
		async delete(owner) {
		...
		async deleteOwner(kind, id) {
			...
			for (const pointer of pointers.filter(value => value.startsWith(`${prefix}:`))) await deleteChunkPointer(backend, pointer);
		},
	};
}
...
async function deleteChunkPointer(backend: Backend, pointer: string, owner?: PersistedChunkOwner): Promise<void> {
	const generations = `${pointer}:generations`;
...
class RedisSubmissionStore implements AgentSubmissionStore {
	...
	async hasUnsettledSubmissions(): Promise<boolean> {
		return integer(await this.backend.command('ZCARD', [this.backend.keys.submissionStatus('queued')])) > 0 || integer(await this.backend.command('ZCARD', [this.backend.keys.submissionStatus('running')])) > 0;
	}
	...
	async beginTurnJournal(input: CreateTurnJournalInput): Promise<boolean> {
		const now = Date.now();
		return integer(await this.backend.eval(journalScript, [this.backend.keys.journal(input.submissionId), this.backend.keys.journals()], [
			'begin', input.submissionId, input.sessionKey, input.kind, input.attemptId, input.operationId,
			input.turnId, input.phase, now, input.checkpointLeafId ?? empty, optionalJson(input.toolRequest),
		])) === 1;
	}

	async updateTurnJournalPhase(attempt: SubmissionAttemptRef, phase: AgentTurnJournalPhase, options: { checkpointLeafId?: string; toolRequest?: unknown; streamKey?: string } = {}): Promise<boolean> {
		return integer(await this.backend.eval(journalScript, [this.backend.keys.journal(attempt.submissionId)], [
			'phase', attempt.attemptId, phase, Date.now(), options.checkpointLeafId ?? empty,
			optionalJson(options.toolRequest), options.streamKey ?? empty,
		])) === 1;
	}

	async commitTurnJournal(attempt: SubmissionAttemptRef, committedLeafId: string): Promise<boolean> {
		return integer(await this.backend.eval(journalScript, [this.backend.keys.journal(attempt.submissionId)], ['commit', attempt.attemptId, Date.now(), committedLeafId])) === 1;
	}

	async markStreamConsumed(attempt: SubmissionAttemptRef, streamKey: string): Promise<boolean> {
		return integer(await this.backend.eval(journalScript, [this.backend.keys.journal(attempt.submissionId)], ['consumed', attempt.attemptId, streamKey, Date.now()])) === 1;
	}

	async appendStreamChunkSegment(streamKey: string, segmentIndex: number, body: string): Promise<boolean> {
		const inserted = integer(await this.backend.command('HSETNX', [this.backend.keys.streamSegments(streamKey), String(segmentIndex), body])) === 1;
		if (inserted) await this.backend.command('SADD', [this.backend.keys.streamSegmentKeys(), streamKey]);
		return inserted;
	}
	...
	async getStreamChunkSegments(streamKey: string): Promise<Array<{ segmentIndex: number; body: string }>> {
		...
		return Object.entries(row).map(([index, body]) => ({ segmentIndex: integer(index), body })).sort((a, b) => a.segmentIndex - b.segmentIndex);
	}
	...
	async replaceTurnJournalAttempt(attempt: SubmissionAttemptRef, nextAttemptId: string, lease?: { ownerId: string; leaseExpiresAt: number }): Promise<AgentSubmission | null> {
		const updated = integer(await this.backend.eval(replaceAttemptScript, [this.backend.keys.submission(attempt.submissionId), this.backend.keys.journal(attempt.submissionId)], [
			attempt.attemptId, nextAttemptId, Date.now(), lease?.ownerId ?? empty, lease?.leaseExpiresAt ?? 0,
		])) === 1;
	...
	async admitDirect(input: DirectAgentSubmissionInput): Promise<AgentSubmission> {
		...
		if (admission.kind !== 'submission') throw new TypeError('Internal direct admission returned an unexpected result.');
	...
	async claimSubmission(claim: SubmissionClaimRef): Promise<AgentSubmission | null> {
		...
		const now = Date.now();
		try {
			const result = await this.backend.eval(claimSubmissionScript, [
				this.backend.keys.submission(claim.submissionId), this.backend.keys.sessionUnsettled(row.sessionKey),
				this.backend.keys.submissionStatus('queued'), this.backend.keys.submissionStatus('running'),
			], [empty, claim.attemptId, now, DURABILITY_DEFAULT_MAX_ATTEMPTS, claim.ownerId, claim.leaseExpiresAt, now + DURABILITY_DEFAULT_TIMEOUT_MS, claim.submissionId]);
	...
	markSubmissionInputApplied(attempt: SubmissionAttemptRef, durability?: { maxRetry: number; timeoutAt: number }): Promise<boolean> {
		const now = Date.now();
		return this.lifecycle(attempt, 'input', now, durability?.maxRetry ?? DURABILITY_DEFAULT_MAX_ATTEMPTS, durability?.timeoutAt ?? now + DURABILITY_DEFAULT_TIMEOUT_MS);
	}
	...
	failSubmission(attempt: SubmissionAttemptRef, error: unknown): Promise<boolean> {
		return this.lifecycle(attempt, 'settle', Date.now(), error instanceof Error ? error.message : String(error));
	}
	...
	async insertAttemptMarker(attempt: SubmissionAttemptRef): Promise<void> {
		...
		await this.backend.command('HSETNX', [this.backend.keys.marker(attempt.submissionId, attempt.attemptId), 'createdAt', Date.now()]);
	...
	async deleteAttemptMarker(attempt: SubmissionAttemptRef): Promise<void> {
		const member = `${encodeSegment(attempt.submissionId)}.${encodeSegment(attempt.attemptId)}`;
		await this.backend.command('DEL', [this.backend.keys.marker(attempt.submissionId, attempt.attemptId)]);
	...
	async listAttemptMarkers(): Promise<AgentAttemptMarker[]> {
		...
		for (const member of members) {
			...
			const createdAt = await this.backend.command('HGET', [this.backend.keys.marker(submissionId, attemptId), 'createdAt']);
			if (createdAt != null) output.push({ submissionId, attemptId, createdAt: integer(createdAt) });
		}
		return output;
	}
	...
	async renewLeases(ownerId: string, submissionIds: string[]): Promise<void> {
		...
		await this.backend.eval(renewLeasesScript, submissionIds.map(id => this.backend.keys.submission(id)), [ownerId, Date.now() + LEASE_DURATION_MS]);
	}
	...
	deleteSession(sessionKey: string, deleteSessionTree: () => Promise<void>): Promise<void> {
		return deduplicateSessionDeletion(this.pendingSessionDeletions, sessionKey, () => this.runSessionDeletion(sessionKey, deleteSessionTree));
	}
	...
	private async admitSubmission(input: DispatchAgentSubmissionInput | DirectAgentSubmissionInput): Promise<AgentDispatchAdmission> {
		const prepared = input.kind === 'direct' ? prepareDirectSubmission(input) : { value: input, chunks: [] };
		const generation = randomUUID();
		...
		await this.backend.command('ZADD', [this.backend.keys.submissionGenerations(input.submissionId), Date.now(), generation]);
		try {
			await stageHash(this.backend, generationKey, [
				['payload', json(prepared.value)], ['chunkCount', String(prepared.chunks.length)], ['sessionKey', sessionKey],
				...prepared.chunks.map((chunk, index) => [`chunk:${index}`, json(chunk)] as [string, string]),
			]);
		} catch (error) {
			await this.backend.command('DEL', [generationKey]);
			throw error;
		...
		if (integer(await this.backend.command('EXISTS', [this.backend.keys.deletion(sessionKey)])) === 1) {
			await this.backend.command('DEL', [generationKey]);
			await this.backend.command('ZREM', [this.backend.keys.submissionGenerations(input.submissionId), generation]);
			throw new TypeError('Durable agent submission admission is unavailable while this session is being deleted.');
		}
		let result: string[];
		try {
			result = strings(await this.backend.eval(admitSubmissionScript, [
				generationKey, this.backend.keys.submission(input.submissionId), this.backend.keys.sequence(),
				this.backend.keys.submissionIds(), this.backend.keys.submissionOrder(), this.backend.keys.submissionStatus('queued'),
				this.backend.keys.sessionSubmissions(sessionKey), this.backend.keys.submissionGenerations(input.submissionId),
				this.backend.keys.receipt(input.submissionId), this.backend.keys.deletion(sessionKey), this.backend.keys.sessionUnsettled(sessionKey),
			], [input.submissionId, sessionKey, input.kind, acceptedAt, DURABILITY_DEFAULT_MAX_ATTEMPTS, generation, Date.now()]));
		} catch (error) {
			if (integer(await this.backend.command('EXISTS', [this.backend.keys.submission(input.submissionId)])) === 0) {
				await this.backend.pipeline([
					...
					{ command: 'ZREM', args: [this.backend.keys.submissionStatus('queued'), input.submissionId] },
					{ command: 'ZREM', args: [this.backend.keys.sessionSubmissions(sessionKey), input.submissionId] },
					{ command: 'ZREM', args: [this.backend.keys.sessionUnsettled(sessionKey), input.submissionId] },
				]);
			}
			throw error;
		}
		if (result[0] === 'created') {
			...
			await reclaimGenerations(this.backend, this.backend.keys.submission(input.submissionId), this.backend.keys.submissionReaders(input.submissionId), this.backend.keys.submissionGenerations(input.submissionId), value => this.backend.keys.submissionGeneration(input.submissionId, value));
			return { kind: 'submission', submission };
		}
		await this.backend.command('DEL', [generationKey]);
		await this.backend.command('ZREM', [this.backend.keys.submissionGenerations(input.submissionId), generation]);
		if (result[0] === 'receipt') return { kind: 'retained_receipt', receipt: { submissionId: input.submissionId, acceptedAt: integer(result[1]) } };
		if (result[0] === 'deleting') throw new TypeError('Durable agent submission admission is unavailable while this session is being deleted.');
		const existingRow = await this.backend.hgetall(this.backend.keys.submission(input.submissionId));
		if (existingRow.sessionKey && existingRow.sessionKey !== sessionKey) return { kind: 'conflict' };
	...
	private async lifecycle(attempt: SubmissionAttemptRef, operation: string, value: RedisArgument, extra1: RedisArgument = empty, extra2: RedisArgument = empty): Promise<boolean> {
		...
		try {
			const result = await this.backend.eval(lifecycleScript, [
				this.backend.keys.submission(id), this.backend.keys.submissionStatus('queued'),
				this.backend.keys.submissionStatus('running'), this.backend.keys.submissionStatus('settled'),
	...
	private async repairSubmissionIndexes(id: string): Promise<void> {
		...
		const commands = ['queued', 'running', 'settled'].map(status => ({ command: status === row.status ? 'ZADD' : 'ZREM', args: status === row.status ? [this.backend.keys.submissionStatus(status), sequence, id] : [this.backend.keys.submissionStatus(status), id] }));
		if (row.status === 'settled') commands.push({ command: 'ZREM', args: [this.backend.keys.sessionUnsettled(row.sessionKey), id] });
		else commands.push({ command: 'ZADD', args: [this.backend.keys.sessionUnsettled(row.sessionKey), sequence, id] });
	...
	private async runSessionDeletion(sessionKey: string, deleteSessionTree: () => Promise<void>): Promise<void> {
		...
		while (true) {
			const now = Date.now();
			const result = strings(await this.backend.eval(acquireDeletionScript, [
				this.backend.keys.deletion(sessionKey), this.backend.keys.sessionUnsettled(sessionKey),
				this.backend.keys.sequence(), this.backend.keys.deletions(),
			], [ownerId, now, sessionKey, now + DELETION_LEASE_MS]));
			if (result[0] === 'active') throw new TypeError('Session cannot be deleted while durable agent submissions are queued or running.');
			if (result[0] === 'owned') {
				cutoff = integer(result[1]);
				break;
			}
			await new Promise(resolve => setTimeout(resolve, DELETION_POLL_MS));
			if (integer(await this.backend.command('EXISTS', [this.backend.keys.deletion(sessionKey)])) === 0) return;
		}
		const heartbeat = setInterval(() => {
			void this.backend.eval(renewDeletionScript, [this.backend.keys.deletion(sessionKey)], [ownerId, Date.now() + DELETION_LEASE_MS]);
		}, DELETION_LEASE_MS / 3);
		try {
			await deleteSessionTree();
		} catch (error) {
			clearInterval(heartbeat);
			await this.backend.eval(finishDeletionScript, [this.backend.keys.deletion(sessionKey), this.backend.keys.deletions()], [ownerId, sessionKey]);
			throw error;
		}
		clearInterval(heartbeat);
		for (const id of await this.backend.zrange(this.backend.keys.sessionSubmissions(sessionKey))) {
			...
			const generationIds = strings(await this.backend.command('ZRANGE', [this.backend.keys.submissionGenerations(id), 0, -1]));
			const deleted = integer(await this.backend.eval(deleteSubmissionScript, [
				this.backend.keys.submission(id), this.backend.keys.submissionIds(), this.backend.keys.submissionOrder(),
				this.backend.keys.submissionStatus('settled'), this.backend.keys.sessionSubmissions(sessionKey),
				this.backend.keys.sessionUnsettled(sessionKey), this.backend.keys.deletion(sessionKey),
				this.backend.keys.journals(), this.backend.keys.submissionGenerations(id), this.backend.keys.receipt(id),
			], [ownerId, cutoff, id]));
			if (deleted === 1) {
				await this.backend.command('DEL', [this.backend.keys.journal(id), this.backend.keys.submissionReaders(id), ...generationIds.map(value => this.backend.keys.submissionGeneration(id, value))]);
			}
		}
		await this.backend.eval(finishDeletionScript, [this.backend.keys.deletion(sessionKey), this.backend.keys.deletions()], [ownerId, sessionKey]);
	}
	...
	private async readOperationalSubmission(id: string, expectedStatus: 'queued' | 'running'): Promise<AgentSubmission | null> {
		try {
			const submission = await this.getSubmission(id);
			if (!submission || submission.status !== expectedStatus) throw new TypeError('Persisted Redis submission index is inconsistent.');
			return submission;
		} catch (error) {
			const row = await this.backend.hgetall(this.backend.keys.submission(id));
			const sessionKey = row.sessionKey ?? empty;
			await this.backend.eval(quarantineSubmissionScript, [
				this.backend.keys.submission(id), this.backend.keys.submissionStatus('queued'),
				this.backend.keys.submissionStatus('running'), this.backend.keys.sessionUnsettled(sessionKey),
				this.backend.keys.sessionSubmissions(sessionKey), this.backend.keys.submissionStatus('settled'),
			], [id, row.sequence ?? 0, Date.now(), error instanceof Error ? error.message : String(error)]);
			return null;
		}
	}

	private async readSubmissionGeneration(submissionId: string, generation?: string): Promise<{ payload: string; chunks: PersistedChunkRow[] }> {
		...
		const value = await readGeneration(this.backend, this.backend.keys.submission(submissionId), this.backend.keys.submissionReaders(submissionId), item => this.backend.keys.submissionGeneration(submissionId, item), parse);
	...
	private async parseSubmission(row: Hash): Promise<AgentSubmission> {
		...
		return {
			...
			...(row.error ? { error: row.error } : {}), attemptCount: integer(row.attemptCount),
			maxRetry: integer(row.maxRetry), timeoutAt: integer(row.timeoutAt),
			...(row.ownerId ? { ownerId: row.ownerId } : {}), leaseExpiresAt: integer(row.leaseExpiresAt),
		};
	}
}
...
function parseJournal(row: Hash): AgentTurnJournal {
	...
	return {
		...
		committed: row.committed === '1', ...(row.committedLeafId ? { committedLeafId: row.committedLeafId } : {}),
	};
}
...
class RedisRunStore {
	...
	async createRun(input: CreateRunInput): Promise<void> {
		...
		await this.backend.eval(createRunScript, [
			this.backend.keys.run(input.runId), this.backend.keys.runs(), this.backend.keys.runsStatus('active'),
			this.backend.keys.runsWorkflow(input.workflowName), this.backend.keys.runStatuses(),
		], [input.runId, input.workflowName, input.startedAt, optionalJson(input.payload), score(input.startedAt), orderKey]);
	}

	async endRun(input: EndRunInput): Promise<void> {
		const status = input.isError ? 'errored' : 'completed';
		await this.backend.eval(endRunScript, [
			this.backend.keys.run(input.runId), this.backend.keys.runs(), this.backend.keys.runsStatus(status), this.backend.keys.runStatuses(),
		], [input.runId, status, input.endedAt, input.isError ? 1 : 0, input.durationMs, optionalJson(input.result), optionalJson(input.error), `${this.backend.keys.prefix}:runs:status:`]);
	}
	...
	async listRuns(opts: ListRunsOpts = {}): Promise<ListRunsResponse> {
		...
		const index = opts.status ? this.backend.keys.runsStatus(opts.status) : opts.workflowName ? this.backend.keys.runsWorkflow(opts.workflowName) : this.backend.keys.runs();
		const ids = await this.backend.zrange(index, 0, -1, true);
		const records: RunPointer[] = [];
		for (const id of ids) {
			const run = await this.getRun(id);
			if (!run || (opts.status && run.status !== opts.status) || (opts.workflowName && run.workflowName !== opts.workflowName)) continue;
			const startedAt = canonicalStartedAt(run.startedAt);
			if (cursor && cursorStartedAt && (startedAt > cursorStartedAt || (startedAt === cursorStartedAt && run.runId >= cursor.runId))) continue;
...
function pointer(run: RunRecord): RunPointer {
	return { runId: run.runId, workflowName: run.workflowName, status: run.status, startedAt: run.startedAt, ...(run.endedAt ? { endedAt: run.endedAt } : {}), ...(run.isError !== undefined ? { isError: run.isError } : {}), ...(run.durationMs !== undefined ? { durationMs: run.durationMs } : {}) };
}
...
class RedisEventStreamStore implements EventStreamStore {
	...
	async appendEvent(path: string, event: unknown): Promise<string> {
		const result = strings(await this.backend.eval(appendEventScript, [this.backend.keys.event(path), this.backend.keys.eventEntries(path), this.backend.keys.eventOrder(path)], [json(event)]));
	...
	async readEvents(path: string, opts?: { offset?: string; limit?: number }): Promise<EventStreamReadResult> {
		...
		if (raw === 'now') return { events: [], nextOffset: meta.nextOffset, upToDate: true, closed: meta.closed };
